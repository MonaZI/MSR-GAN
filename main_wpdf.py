...
def main(args):
    ...
    if args.periodic:
        sig = sig_from_a(np.random.uniform(size=(args.a_size,))-0.5, args.sig_len)
    ...
    if not os.path.isdir('./results/'+args.expName):
        os.mkdir('./results/'+args.expName)
    meas = meas_gen_pdf(sig, args.mask_len, args.num_meas, pdf, sigma=0.)
    if args.snr==0:
        args.sigma=0
    else:
        args.sigma = np.sqrt(np.var(meas)/args.snr)
...
def arg_parse():
    ...
    parser.add_argument('-snr', type=float, default=1., help='the snr of the projection data')
    parser.add_argument('-correct_pdf', action='store_true', default=False, help='to fix the pdf with the gt pdf')
    # model
    parser.add_argument('-modelSavePath', type=str, default='/home/mona/projects/MSRGan/models', help='path to save the model')
    ...
    parser.add_argument('-a_size', type=int, default=10, help='the size of the random vectors as the coeffs of the signal')
    parser.add_argument('-z_size', type=int, default=8, help='the size of the random vectors input to the shift encoder net')
    parser.add_argument('-z_mid_size', type=int, default=100, help='the size of the first fully connected layer for the shift encoder network')
    ...
    parser.add_argument('-lrate', type=float, default=1e-3, help='the learning rate')
    parser.add_argument('-gamma_lrate', type=float, default=0.9, help='the learning rate')
    parser.add_argument('-lrate_x', type=float, default=1e-3, help='the learning rate for x')
    parser.add_argument('-lrate_pdf', type=float, default=1e-3, help='the learning rate for the encoder')
    parser.add_argument('-lrate_enc', type=float, default=5e-5, help='the learning rate for the encoder')
    ...
    parser.add_argument('-lamb', type=float, default=1e-4, help='the weight of the regularization term')
    parser.add_argument('-lamb_moment', type=float, default=1., help='the weight of the regularization term for the moment term')
    # misc
    parser.add_argument('-moment', action='store_true', default=False, help='use the moment method or not')
    ...
    parser.add_argument('-periodic', action='store_true', default=False, help='if the signal is periodic or not')
    ...
    parser.add_argument('-expName', type=str, default='exp', help='the name of the experiment')
    parser.add_argument('-logPath', type=str, default='./log/', help='the path to the log files')
